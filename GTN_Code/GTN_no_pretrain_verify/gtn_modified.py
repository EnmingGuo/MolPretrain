...
class GTN(nn.Module):   
    def __init__(self, task ,num_channels, w_in, w_out, num_layers, emb_dim, args=None):
        ...
        self.gcn = GCNConv(in_channels=self.emb_dim, out_channels=w_out, args=args)
        self.pool = global_mean_pool
        self.linear = nn.Linear(self.w_out*self.num_channels, self.w_out)       

        self.x_linear = nn.Linear(self.w_in, self.emb_dim)

        edge_embedding_params = [num_bond_type, num_bond_stereo, num_bond_direction, num_is_conjugated]
        edge_embed_layers = []
        for emb in edge_embedding_params:
            edge_embed = nn.Embedding(emb, 1)
            nn.init.xavier_uniform_(edge_embed.weight.data)
            edge_embed_layers.append(edge_embed)
        self.edge_embedding = nn.ModuleList(edge_embed_layers)
    ...
    def forward(self, data, num_nodes=None, eval=False):
        ...
        if len(single_attr) != 0:
            attr_single = 0
            for i, embedding in enumerate(self.edge_embedding):
                attr_single += embedding(single_attr[:,i])
        ...
        if len(double_attr) != 0:
            attr_double = 0
            for i, embedding in enumerate(self.edge_embedding):
                attr_double += embedding(double_attr[:,i])
        ...
        if len(triple_attr) != 0:
            attr_triple = 0
            for i, embedding in enumerate(self.edge_embedding):
                attr_triple += embedding(triple_attr[:,i])
        ...
        if len(aromatic_attr) != 0:
            attr_aromatic = 0
            for i, embedding in enumerate(self.edge_embedding):
                attr_aromatic += embedding(aromatic_attr[:,i])
            A.append((aromatic_index, attr_aromatic.squeeze(-1)))

        # self-loop
        edge_tmp = torch.stack((torch.arange(0,num_nodes),torch.arange(0,num_nodes))).to(device)
        value_tmp = torch.ones(num_nodes,dtype=torch.float32).to(device)
